...
docker run -d --name yora-gateway -p 1520:1520 --hostname yora-gateway     \
 ...
 pachot/yora-gateway
docker run -d --name xe -p 1521:1521                                       \
...
create public database link "yb" connect to "yugabyte" identified by "yugabyte" using '(DESCRIPTION=(ADDRESS=(PROTOCOL=tcp)(HOST=$(docker inspect yora-gateway --format "{{ .NetworkSettings.IPAddress }}"))(PORT=1520))(CONNECT_DATA=(SID=YORA))(HS=OK))';
select "query" as "Oracle query as seen from YugabyteDB" from "pg_stat_activity"@"yb" where "application_name"='yora-gateway';
SQL
} || exit
echo "All works"
